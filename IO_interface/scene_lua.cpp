...
int gr_material_cmd(lua_State* L)
{
  ...
  if(type == 3)
  {
    ...
    uint width = luaL_checknumber(L, 3);
    uint height = luaL_checknumber(L, 4);
    double ks[3];
    get_tuple(L, 5, ks, 3);
    double shininess = luaL_checknumber(L, 6);
    data->material = new PhongMaterial(file_name, width, height, glm::vec3(ks[0], ks[1], ks[2]), shininess, type);
  }
  else
  {
    double kd[3], ks[3];
